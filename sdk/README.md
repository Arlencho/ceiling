...
The signature listing is one request. Transaction reads then run one at a time by default, 1000 ms after the previous RPC read (`concurrency` is how many of those reads run together). A 429 is tried up to 4 times with capped exponential backoff and jitter: the ceiling starts at 200 ms, doubles, and never exceeds 2000 ms, and the built-in wait is between half that ceiling and the ceiling. The SDK opens its own connection for these reads with `disableRetryOnRateLimit`, so the web3 client does not retry the same 429. That connection keeps the caller's commitment, headers, fetch, fetch middleware, http agent, and websocket endpoint.
