...
- Phone one is the owner. Phone two has a distinct guardian wallet. Keep both visibly in frame for the three-second cold open and for Hold. Use a separate first-run take for the eight-second Protect cut-in.
...
Keep 00:48 to 01:35 only after the trade program, SDK, app and rehearsal script agree on the deployed version. The trade instruction (#286) and the SDK trade client (#291) are on main. The devnet program was last upgraded 2026-09-25T12:18Z (docs/DEVNET.md), before #286 merged, so the deployed program has no trade instruction until the next upgrade. The reason mapping in `programs/veto/src/trade_state.rs` is 11 destination, 12 pool, 13 daily allowance, 14 price floor. Main has only trade reason constants in `app/lib/constants.ts`; the trade rule and refusal screens require `feat/app-trade-rule` ([PR 294](https://github.com/Arlencho/veto/pull/294)). Inspect the merged code again; merged code and an existing pool do not establish deployment.
...
If the upgrade or rehearsal misses the gate, remove the entire 47 seconds. Do not replace it with a local validator, mockup, terminal-only refusal or a future-feature card.
...
Use a fresh agent identity so discovery cannot select an earlier active rule. The connect command asks for missing rule fields before it prints the QR. Pre-answer every rule prompt to fit the 35-second beat:
...
| Claim | Source or required branch |
|---|---|
