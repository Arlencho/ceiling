// Security round 2, PR 213 (issue 156). Round 1 proved one shape, an omitted
// logMessages key beside a listed CPI, printed VERDICT: CONFIRMED for the file
...
// lookup table and the node omitted loadedAddresses, which web3.js allows.
function relayPaidLoadedOmitted(mandate: PublicKey, ledger: PublicKey, row: Row, slot: number) {
...
for (const shape of shapes) {
  test(`a no-mandate date_range that omits a CPI payment with ${shape.name} is not checked`, async () => {
    ...
    const txs = new Map<string, unknown>([
      ...
      [HIDDEN, shape.hidden(mandate, ledger, hidden, 3)],
    ]);
    const programListed = [
