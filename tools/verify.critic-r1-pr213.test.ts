// Critic round 1, PR 213 (issue 156). The relay body below is the one a node
// with recording off returns: innerInstructions null beside logMessages null.
// The fix reads the CPI list to decide whether the program was invoked, so
// this payment is neither a decision nor a gap, and the file that omits it
// prints VERDICT: CONFIRMED. Copied from verify.issue-156.test.ts with two
// lines changed (the meta shape and the test name).
...
test("a no-mandate date_range that omits a CPI payment with null inner instructions and a null log body is not checked", async () => {
  ...
  const txs = new Map<string, unknown>([
    ...
    [HIDDEN, relayPaidNullLogs(mandate, ledger, hidden, 3)],
  ]);
  const programListed = [
  ...
  assert.match(result.text, /null log body/);
