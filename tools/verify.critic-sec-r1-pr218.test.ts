...
// object. This file runs the no-mandate date_range check through
// assessBundle with the hidden relay reachable only through an address
// table, across the loadedAddresses shapes web3.js lets through, on the
// listed path and the block-scan path. A missing log body must print not
// checked; a present Paid frame must reject the file that omits it; neither
// may print VERDICT: CONFIRMED.
...
const SHORT = { writable: [], readonly: [] };
const RESOLVED = { writable: [], readonly: [PROGRAM] };
...
const shapes: { name: string; meta: Meta; expect: Expect }[] = [
  { name: "loadedAddresses omitted and a null log body", meta: () => ({ logMessages: null }), expect: "unchecked" },
  { name: "loadedAddresses short and a null log body", meta: () => ({ loadedAddresses: SHORT, logMessages: null }), expect: "unchecked" },
  { name: "loadedAddresses short and no logMessages key", meta: () => ({ loadedAddresses: SHORT }), expect: "unchecked" },
  { name: "loadedAddresses resolved and a null log body", meta: () => ({ loadedAddresses: RESOLVED, logMessages: null }), expect: "unchecked" },
  { name: "loadedAddresses omitted and the Paid frame", meta: (m, r) => ({ logMessages: relayLogs(m, r) }), expect: "rejected" },
  { name: "loadedAddresses short and the Paid frame", meta: (m, r) => ({ loadedAddresses: SHORT, logMessages: relayLogs(m, r) }), expect: "rejected" },
  { name: "loadedAddresses resolved and the Paid frame", meta: (m, r) => ({ loadedAddresses: RESOLVED, logMessages: relayLogs(m, r) }), expect: "rejected" },
];
